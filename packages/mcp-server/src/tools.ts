...
export const ARCHON_MCP_TOOL_DEFINITIONS: ArchonToolDefinition[] = [
    ...
    tool({ name: 'archon_check_did', cliCommand: 'check-did', description: 'Inspect known DID document problems and proposed repairs without changing the DID.', schema: DidSchema, handler: (runtime, { did }) => requireKeymaster(runtime).checkDID(did) }),
    tool({ name: 'archon_repair_did', cliCommand: 'repair-did', description: 'Apply supported DID document repairs after inspecting them with archon_check_did.', schema: DidSchema.merge(ConfirmSchema), mutates: true, handler: (runtime, { did }) => requireKeymaster(runtime).repairDID(did) }),
    ...
    tool({ name: 'archon_publish_assertion_key', cliCommand: 'publish-assertion-key', description: 'Publish an Ed25519 assertion key (Multikey) so credentials this ID issues carry a proof other DID methods can verify.', schema: z.object({ name: z.string().optional() }), mutates: true, handler: (runtime, { name }) => requireKeymaster(runtime).publishAssertionKey(name) }),
    tool({ name: 'archon_unpublish_assertion_key', cliCommand: 'unpublish-assertion-key', description: 'Remove the Ed25519 assertion key from the current ID.', schema: z.object({ name: z.string().optional() }), mutates: true, handler: (runtime, { name }) => requireKeymaster(runtime).unpublishAssertionKey(name) }),
    ...
    tool({ name: 'archon_receive_didcomm', cliCommand: 'receive-didcomm', description: 'Fetch and unpack queued DIDComm messages from the current ID\'s mailbox. Acknowledges (removes) them unless ack is false.', schema: z.object({ name: z.string().optional(), endpoint: z.string().optional(), ack: z.boolean().optional() }), mutates: true, handler: (runtime, { name, endpoint, ack }) => requireKeymaster(runtime).receiveDidComm(compactOptions({ name, endpoint, ack })) }),
    tool({ name: 'archon_ack_didcomm', cliCommand: 'ack-didcomm', description: 'Acknowledge (remove) DIDComm messages from the current ID\'s mailbox by id.', schema: z.object({ ids: z.array(z.string()), name: z.string().optional(), endpoint: z.string().optional() }), mutates: true, handler: (runtime, { ids, name, endpoint }) => requireKeymaster(runtime).ackDidComm(ids, compactOptions({ name, endpoint })) }),
    tool({ name: 'archon_send_credential_didcomm', cliCommand: 'send-credential-didcomm', description: 'Deliver an issued credential over DIDComm. Unlike send_credential, this reaches subjects whose DID is not a did:cid, because it carries the credential itself rather than a reference.', schema: z.object({ did: z.string(), to: z.union([z.string(), z.array(z.string())]), comment: z.string().optional(), anoncrypt: z.boolean().optional(), name: z.string().optional() }), mutates: true, handler: (runtime, { did, to, comment, anoncrypt, name }) => requireKeymaster(runtime).sendCredentialDidComm(did, to, compactOptions({ comment, anoncrypt, name })) }),
    tool({ name: 'archon_accept_credential_didcomm', cliCommand: 'accept-credential-didcomm', description: 'Accept a credential received over DIDComm into the wallet. Returns false for a credential from a foreign issuer, which has no did:cid to resolve.', schema: z.object({ message: JsonObjectSchema }), mutates: true, handler: (runtime, { message }) => requireKeymaster(runtime).acceptCredentialDidComm(message) }),
