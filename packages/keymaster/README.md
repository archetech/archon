...
| `ARCHON_WALLET_PATH` | No | `~/.archon/wallet.json`, or `./wallet.json` if one is already there | Path to wallet file. The default follows you between directories; an explicit path is obeyed even if nothing is there |
