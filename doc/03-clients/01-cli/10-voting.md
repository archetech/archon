...
The Archon protocol allows for near-endless forms of communication. This page covers the "group" and "poll" set of commands provided by the `archon` CLI tool, which demonstrate how Archon can be used to perform secure voting.
...
1. Create a new group:
    ...
    ```sh
    archon group-create icecream-tasters
    did:cid:klf75KJH6LKlh654LP4C7hexSVfDhFcSiZr8xfS1tg
    ```

1. Add members to the polling group using their DID:

    ```sh
    archon group-add icecream-tasters did:cid:z3v8AuzzXKfwrt4Y3AAbDaGqLNgyn1BDhP7wUFpEMEngmwYwjm8
    {
      members: [
        'did:cid:z3v8AuzzXKfwrt4Y3AAbDaGqLNgyn1BDhP7wUFpEMEngmwYwjm8'
      ],
   ...
   Members can also be added using their [aliased names](./aliased-names):
    ...
    ```sh
    archon group-add icecream-tasters alice
    {
      members: [
        'did:cid:z3v8AuzzXKfwrt4Y3AAbDaGqLNgyn1BDhP7wUFpEMEngmwYwjm8',
        'did:cid:z3v8AuaZ6U4FwcfLA82aGf6n8qpwRtkKCStRMokvU4gSwHHHrzC'
      ],
      name: 'icecream-tasters'
    }
    ```

    If a member should be removed from a group, use `archon group-remove` with their DID or aliased name.

1. You can check to see if a DID is part of the group:

    ```sh
    archon group-test icecream-tasters bob
    false
    ```
...
1. Create a JSON file to define the poll using the `poll-template` command:
    ...
    ```sh
    archon poll-template > icecream-poll.json
    ```
...
1. Edit the poll file:
    ...
    ```json title="icecream-poll.json"
    {
        ...
        "roster": "did:cid:klf75KJH6LKlh654LP4C7hexSVfDhFcSiZr8xfS1tg",
        "options": [
            "Chocolate",
            "Vanilla",
            "Strawberry",
...
1. Using that file, create the poll:
    ...
    ```sh
    archon poll-create icream-poll.json best-icecream-flavor
    ```

1. Anyone can now view the poll using the DID or an aliased name (if they create one locally):

    ```json
    archon poll-view best-icecream-flavor
    {
        "description": "Which flavor of ice cream is the best?",
        "options": [
            "Chocolate",
...
1. Members of the group can cast their vote using `poll-vote`:
    ...
    ```sh
    archon poll-vote best-icecream-flavor 5
    did:cid:z3v8AuaYFc3SZXkXyYxBxdGb1EuC5hV2BcjfMhemYMg56ztyAJx
    ```
...
1. Once vote DIDs are collected, the poll controller can add them to the poll results with `poll-update`, using only the vote's (not the voter's) DID:
    ...
    ```sh
    archon poll-update did:cid:z3v8AuaYFc3SZXkXyYxBxdGb1EuC5hV2BcjfMhemYMg56ztyAJx
    OK
    ```
...
1. The controller of the poll can publish the results with `poll-publish`:
    ...
    ```sh
    archon poll-publish best-icecream-flavor
    OK
    ```
...
1. To make the votes in a poll public use `poll-reveal`:
    ...
    ```sh
    archon poll-reveal best-icecream-flavor
    OK
    ```
