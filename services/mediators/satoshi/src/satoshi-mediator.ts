...
const satoshiImportStalledHeight = new promClient.Gauge({
    name: 'satoshi_import_stalled_height',
...
const chain: ChainReader = {
    header: (hash) => getChainBlockHeader(hash),
    hashAt: (height) => getChainBlockHash(height),
...
async function importBatches(): Promise<boolean> {
    ...
    for (const item of db.discovered) {
        ...
        try {
            ...
            const stalledError = `${error.error || JSON.stringify(error)}`;
            await jsonPersister.updateDb((db) => {
    ...
    satoshiImportStalledHeight.set(stalledAt ?? 0);
