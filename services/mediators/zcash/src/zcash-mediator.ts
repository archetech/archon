...
const zcashAnchorFailures = new promClient.Counter({
    name: 'zcash_anchor_failures_total',
...
async function importBatches(): Promise<boolean> {
    ...
    for (const item of db.discovered) {
        ...
        try {
            ...
            zcashImportErrors.inc();
            const importError = String(error?.error ?? error?.message ?? error);
...
async function anchorBatch(): Promise<void> {
    ...
    try {
        ...
        if (operations.length > 0) {
            ...
            try {
                txid = await createOpReturnTxn(anchoredDid);
            } catch (error: any) {
            ...
            if (txid) {
                ...
                await jsonPersister.updateDb(async (data) => {
