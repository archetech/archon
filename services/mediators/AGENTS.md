...
- An anchored batch DID commits to the signed create operation and its original ordered `data.batch.ops` list. Use `gatekeeper.getGenesis(did)` for imports; this checks content identity/shape without controller authorization or history mutation. Do not use latest or versioned authorized DID resolution to interpret a batch anchor. Keep missing genesis content retryable and authorize every contained operation normally. The original proof is preserved as provenance, not treated as an authorization verdict by retrieval (#1269).
