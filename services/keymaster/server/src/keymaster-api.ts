 ...
 *                   type: string
 */
v1router.post('/ids/:id/change-registry', async (req, res) => {
    try {
        const { registry } = req.body;
        const ok = await keymaster.changeRegistry(req.params.id, registry);
        res.json({ ok });
    } catch (error: any) {
        res.status(400).send({ error: error.toString() });
    }
...
const server = app.listen(port, config.bindAddress, async () => {
    ...
    keymaster = new Keymaster({
        ...
        didcommServiceURL: config.didcommGatewayURL || undefined,
    });
