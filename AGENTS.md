...
- Do not enable npm provenance/OIDC in the publish workflow unless every published package has matching npm trusted publisher configuration for the workflow filename and GitHub environment; otherwise token-based recovery publishes can fail with npm `E404 Not found` after OIDC token exchange.
