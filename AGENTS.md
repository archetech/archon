...
- Solana mediator scan cursors are slots, but Gatekeeper registration/block metadata should use produced Solana block heights. Keep `ARCHON_SOL_START_BLOCK` as the import/register floor and treat `ARCHON_SOL_START_SLOT` only as a legacy scan paging hint.
