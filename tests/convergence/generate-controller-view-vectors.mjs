...
for (const legacy of [false, true]) {
    ...
    const genesis = sign({ type: 'create', created: date(1),
        registration: { version: 1, type: 'agent', registry: 'hyperswarm' }, publicJwk: keys[0].publicJwk }, 0, date(1));
    ...
    for (const mode of ['wrong-registry-suffix', 'confirmed-suffix', 'missing-registration']) {
        ...
        vectors.push({ legacy, mode, did, assetDid, events, controllerIds: await Promise.all([genesis, migration, update].map(cid)),
