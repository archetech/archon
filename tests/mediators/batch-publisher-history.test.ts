...
it.each(['satoshi', 'zcash', 'ethereum', 'solana'])('%s still rejects an unauthorized contained operation', async name => {
    const db = new Db(name);
    const ipfs = new MemoryClient();
    const gatekeeper = new Gatekeeper({ db, ipfs });
    const operations = [fixture.publisher, fixture.owner, fixture.target];
    for (const operation of [...operations, fixture.invalidOperation, fixture.invalidBatch]) {
        await ipfs.addJSON(operation, { canonical: true });
    }
    await gatekeeper.importBatch(operations.map(hint));
    await gatekeeper.processEvents();
    // The batch is genuinely signed; only its contained operation is invalid.
    expect(await gatekeeper.verifyOperation(fixture.invalidBatch)).toBe(true);
    const result = await importer(name, gatekeeper)({
        ...fixture.metadata.registration, did: fixture.invalidBatchDid, time: fixture.metadata.time,
        batchHash: `0x${createHash('sha256').update(fixture.invalidBatchDid).digest('hex')}`,
    });
    expect(result.processed.rejected).toBe(1);
    expect((await gatekeeper.resolveDID(fixture.targetDid)).didDocumentData).toEqual(fixture.target.data);
});
