...
This is a start of the CLI testing suite for Archon. More tests will be added here. But to get started, these tests are based on Expect/TCL. These tests should be ran only on nodes that do not have any mediators. This will ensure nothing is written.
...
- Before running tests, the tests expect the cli commands are accessible by running the "archon" or "admin" command globally. This can be done by adding them to your PATH.
- Then from /tests, run "./run_cli_tests.sh --local"

* When running without hyperswarm you should set ARCHON_DEFAULT_REGISTRY=local in the .env, otherwise create operations will fail.
