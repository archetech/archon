...
use config::Config;
use events::{handle_did_operation, import_batch_impl, process_events_impl, ImportBatchResult, ImportEventsResult, ImportStatus, ProcessEventsResult, queue_outbound_operation};
...
use proofs::{
    ...
    verify_create_operation_impl, verify_event_shape, verify_operation_impl, verify_update_operation_impl,
};
use resolver::{
    check_dids_impl, log_status_snapshot, query_docs_impl, refresh_metrics_snapshot, resolve_local_doc_async,
    search_docs_impl, start_background_tasks, update_metrics_from_check, verify_db_impl, CheckDidsByType,
    CheckDidsResult, VerifyDbResult,
};
use store::{
    chrono_like_now, compare_ordinals, event_record_to_value, expected_registry_for_index, hydrate_redis_event,
    redis_event_to_stored_value, value_to_event_record, BlockLookup, DbBackend, EventRecord, GatekeeperDb,
    JsonDb, JsonDbFile, ResolveOptions, ResolvedDoc,
};
...
mod tests {
    ...
    fn hydrate_redis_event_restores_operation_from_op_store() {
        ...
        assert_eq!(event.opid.as_deref(), Some("bagaaiera4yheu3faxmc7kqpwpd3w2uj62psmy3giczzyrdf2ulnckg2e3hnq"));
    ...
    fn redis_event_storage_strips_embedded_operation_when_opid_present() {
        ...
        assert_eq!(stored.get("opid").and_then(Value::as_str), Some("bagaaieratestopid"));
        assert_eq!(stored.get("ordinal").and_then(Value::as_array).map(|a| a.len()), Some(2));
    }
    ...
    fn redis_event_storage_keeps_operation_when_no_opid_exists() {
        ...
        assert_eq!(stored.get("operation").and_then(|v| v.get("type")).and_then(Value::as_str), Some("create"));
    ...
    fn canonical_json_and_cid_match_shared_deterministic_vector() {
        let vectors: Value = serde_json::from_str(include_str!("../../../../tests/gatekeeper/deterministic-vectors.json"))
            .expect("vectors should decode");
        let local_agent = &vectors["localAgent"];
        let operation = &local_agent["operation"];

        assert_eq!(canonical_json(operation), local_agent["canonical"].as_str().unwrap());
        assert_eq!(
            generate_json_cid(operation).expect("cid generation should work"),
            local_agent["cid"].as_str().unwrap()
        );
        assert_eq!(
            generate_did_from_operation(&test_config(), operation).expect("did generation should work"),
    ...
    fn verify_proof_format_accepts_valid_and_rejects_invalid_vectors() {
        ...
        assert!(verify_proof_format(vectors["agentCreateValid"]["operation"].get("proof")));
        assert!(verify_proof_format(vectors["agentCreateInvalidProof"]["operation"].get("proof")));
    ...
    fn list_dids_and_resolve_doc_preserve_version_and_delete_semantics() {
        ...
        assert_eq!(resolved["didDocumentMetadata"]["deleted"], "2026-04-11T12:10:00Z");
