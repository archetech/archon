    ...
    for vector in vectors {
        ...
        for case in vector["cases"].as_array().unwrap() {
            ...
            let events: Vec<Value> = case["order"]
                .as_array().unwrap().iter().enumerate().map(|(receipt, index)| {
                    let index = index.as_u64().unwrap() as usize;
                    let operation = &vector["operations"][index];
                    let hyperswarm = vector["registry"] == "hyperswarm";
                    let ordinal = if vector["receipts"] == "fresh" { receipt } else if vector["receipts"] == "tied" { 0 } else { index };
                    let mut event = json!({
                        "operation": operation, "registry": vector["registry"],
                        "time": if hyperswarm { json!("2026-09-10T00:00:00Z") } else { operation["proof"]["created"].clone() },
        ...
        let expected: BTreeSet<_> = if vector["name"] == "hyperswarm/fresh/fork"
            || vector["name"] == "hyperswarm/tied/fork"
        {
            ["0,1,3".to_string(), "0,2".to_string()]
                .into_iter()
                .collect()
        } else {
            ["0,1,3".to_string()].into_iter().collect()
        };
        assert_eq!(outcomes, expected, "{}", vector["name"]);
    }
}
