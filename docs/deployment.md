...
Each subservice has its own profile, added on top of `drawbridge`:

| Profile | Brings in | Enables |
|---------|-----------|---------|
| `drawbridge` | `drawbridge`, `drawbridge-client` | The front-door proxy itself |
| `drawbridge-lightning` | `lightning-mediator` + the Lightning rail (CLN, LNbits, RTL, init containers) | L402 paywall, zaps, `/invoice/:did` |
| `drawbridge-names` | `herald`, `herald-client` | `/names/*`, `/.well-known/*` |
| `drawbridge-tor` | `tor` | Onion hidden service |

The full stack — equivalent to the pre-split `drawbridge` profile:

```env
COMPOSE_PROFILES=hyperswarm,drawbridge,drawbridge-lightning,drawbridge-names,drawbridge-tor
```
...
Enable **`drawbridge-lightning`**, which starts the bundled Lightning dependency stack (CLN, LNbits, RTL) *and* `lightning-mediator`. Runes and secrets are auto-generated by init containers and shared with `lightning-mediator` via Docker volumes — no manual configuration needed.

Two things to be aware of after the profile split:

- `drawbridge` alone no longer brings in Lightning. Without `drawbridge-lightning`, L402 invoice creation has no mediator to talk to.
- The bare `lightning` profile starts the rail (CLN, LNbits, RTL and their init containers) but **not** `lightning-mediator`, which is declared only under `drawbridge-lightning`. A `lightning`-only deployment therefore has no service consuming the rail; use `drawbridge-lightning` unless you are driving CLN/LNbits directly by other means.
