...
Enable the `lightning` profile, or `drawbridge-lightning`, which starts the bundled Lightning dependency stack alongside the mediator. Runes and secrets are auto-generated by init containers and shared with `lightning-mediator` via Docker volumes — no manual configuration needed. Note that `drawbridge` alone no longer brings in Lightning: without `drawbridge-lightning`, L402 invoice creation has no mediator to talk to.
