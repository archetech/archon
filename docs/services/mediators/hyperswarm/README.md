...
`ARCHON_PROTOCOL` ships with the release, in
`services/mediators/hyperswarm/src/config.js`. All nodes that want to peer
...
| `ARCHON_PROTOCOL` | ships with the release | Hyperswarm topic seed. Must match peers you want to talk to, so pinning it opts out of future changes. |
